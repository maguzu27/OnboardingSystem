...
from Admin_Page.Setup_Alerts.Admin_Setup_Alert_Items import AlertItemsManager
from Admin_Page.Job_Requirements.Admin_JobReq_Items import RequirementItemsEditor

class AlertsSetupManager(QWidget):
    ...
    def init_ui(self):
        ...
        # Header
        header = QHBoxLayout()
        header.addWidget(QLabel("<h2>Alerts Setup</h2>"))
        ...
        self.add_btn = QPushButton("Add New Alerts")
        ...
        self.table.setHorizontalHeaderLabels([
            "ID", "Alert Name", "Alert Code","Type", "Description", "Status", "Date Triggered", "Acknowledged By", "Acknowledged Date"
        ])
    ...
    def load_data(self):
        ...
        data = self.db.get_all_alert_dashboard_items() 
        ...
        for r_idx, row in enumerate(data):
            ...
            self.table.setItem(r_idx, 2, QTableWidgetItem(str(row[2]))) # Alert Code
            self.table.setItem(r_idx, 3, QTableWidgetItem(str(row[3]))) # Alert Type
            self.table.setItem(r_idx, 4, QTableWidgetItem(str(row[4]))) # Alert Description
            self.table.setItem(r_idx, 5, QTableWidgetItem(str(row[5] or ""))) # Alert Status
            self.table.setItem(r_idx, 6, QTableWidgetItem(str(row[6] or ""))) # Date Triggered
            self.table.setItem(r_idx, 7, QTableWidgetItem(str(row[7] or ""))) # Acknowledged By
            self.table.setItem(r_idx, 8, QTableWidgetItem(str(row[8] or ""))) # Acknowledged Date

    def add_record(self):
        # Pass the database, the user string, and the parent (self)
        # Note: We don't really need a back_callback for a Popup dialog
        dialog = AlertItemsManager(self.db, self.current_user, self)
